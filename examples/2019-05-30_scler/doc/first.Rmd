---
# Diversity Dynamics of scleractinian corals with divDyn 0.7.1
...
The purpose of this vignette is to guide new users through data acquisition and analyse, as well as to demonstrate the basic capabilities of the divDyn R package. This tutorial will use Scleractininan corals as an example group which was selected selected because the due to data quality reasons and as we have readily available additional information about the coral taxa with which to customize the analyses. 


# Data download
To run analyses occurrence data have to be downloaded and processed first. You can download all occurrences of Scleractininan corals, if you paste the following lines of code to your browser.

```
https://paleobiodb.org/data1.2/occs/list.csv?interval=Ediacaran,Holocene&base_name=Scleractinia&
show=class,classext,genus,subgenus,abund,coll,coords,loc,paleoloc,strat,stratext,lith,
env,ref,crmod, timebins,timecompare
```

The snippet above instructs the Paleobiology Database download API to compile all occurrences of scleractinians (from the Ediacaran to the Holocene, despite), and combine them with a set of predefinied lithological, geographic and startigraphic variables in a comma-separated (.csv) format.
As this format is relatively inefficient it is better practice to store them in a binary format. You can read this file with the following line of code:

```
dat <- read.csv("D:/pbdb_data.csv", sep=",", header=TRUE, stringsAsFactors=FALSE)
```
and save it exactly the way it was imported with a simple ``save`` command:
```
save(dat, file="2019-05-30_scler.RData")
```
...
## Merging the occurrences with the trait table

After you have set up your working directory, you can read the data with the load() function:


```{r load, echo= TRUE, eval=TRUE}
workDir <- "/media/adam/data/shared/Dropbox/WorkSpace/2017-04-05_divDyn/examples/2019-05-30_scler"
setwd(workDir)

load(file="data/2019-05-30_scler.RData")
```

This funciton call will load the data and attach the object to the global namespace with the same name with which it was saved.
Determining the symbotic status of extant corals is straightforward (whether or not they host photosymbionts), but not so for fossil ones. Although the photosymbionts do not fossilize, some morphological traits and geogchemical properties correlate with the corals' ability to host symbionts. W. Kiessling compiled a list of most likely photosymiotic status for coral genera occurring in the Paleobiology Database. This list is available from github and can be loaded directly with this URL. 


```{r symtab, echo= TRUE, eval=TRUE}
sym <- read.csv("data/coral_genus_traits.csv", sep=",", header=TRUE, stringsAsFactors=FALSE)
```
This table also contains information about corallite integrity, coloniality, and whether the taxon is extant or extinct.

Every row in this table represent a coral genus, which is designated with in the column 'genus.proper'. As the corresponding column in the PaleoDB is simply 'genus' it is easier to merge the two dataset if these columns have the same name. Since 'genus' is shorter, keeping that is likely more efficient.


```{r rename, echo= TRUE, eval=TRUE}
# rename the taxon name
	colnames(sym)[colnames(sym)=="genus.proper"] <- "genus"
```

Those entries that have no valid genus names in the PaleoDB can be omitted at this stage. They will not provide us any information about the evolution of the group.
...
Now it is very easy to match the two tables merge function. Stating ``all.x=TRUE`` force the keeping of all rows in the first argument of the merge funciton, which is in our case the PaleoDB. This is handy, as we'd like to assess how many fossil genera do get phototsymbiotic tags.

```{r merge, echo= TRUE, eval=TRUE}
# merge the two tables
	dat <- merge(dat, sym, by="genus", all.x=TRUE)

# which do not get ?
	# occurrences
	sum(is.na(dat$ECOLOGY))/nrow(dat) 
	# genera
	genTab<-unique(dat[, c("genus", "ECOLOGY")])
...
Every colleciton in the the PaleoDB has a user entered 'early_interval' and 'later_interval' field. These are fields are used to place the collection in stratigraphy/geological time. The 'early_interval' collection is mandatory for every entry while the 'late_interval' is only necessary if the taxon does not fit in the interval specified as 'early_interval'. Otherwise the 'late_interval' field is blank (empty quotes: "").

The divDyn package uses two frequently used, discrete time scales that we use as the finest stratiigraphic resolution of our analyses: the stage level time scale ('stages', 95 bins) and the ten million year time scale (bins, 49 bins). These can be attached with the data() function:
*Note that the ``bins`` will be renamed to ``b10s`` in the divDyn 0.8.*
...
This list is designed to be used with the categorize() function of the packag, that combines the large number of entries to and replaces them with the name of the group they belong to. This has to be run on both the 'early_interval' and the 'late_interval' fields.
...
You can get a more organized view of sampling by running the binstat() function in divDyn, that gathers the occurrence, collection, and reference counts in a single line. This is the general use of the high-level function of the package: you state the occurrence data.frame as the first argument and then the column names as additional arguments. The column tax stands for the taxon names, bin for the discrete time bins, coll for collection identifiers and ref for reference identifiers.

```{r bsFull, echo= TRUE, eval=TRUE}
bsFull <- binstat(dats, tax="genus", bin="stg", coll="collection_no", ref="reference_no")
bsFull$occs
```
...
# turnover rates
tp(ylim=c(0,1.5), ylab="per capita turnover rates")	
...
lines(stages$mid, azDD$divRT, lwd=2, col="orange", lty=2)

# legend function
legZAZ <- function(...) 
  legend(legend=c("z", "az"), col=c("blue", "orange"), lty=c(1,2), bg="white", cex=1.3, ...)
legZAZ("topleft")
```
...
lines(stages$mid, azDD$extPC, lwd=2, col="orange", lty=2)
legZAZ("topleft")
```
...
lines(stages$mid, azDD$oriPC, lwd=2, col="orange", lty=2)
legZAZ("topleft")
```
