---
# Diversity Dynamics of scleractinian corals with divDyn 0.8.0
...
```
dat <- read.csv("pbdb_data.csv", sep=",", header=TRUE, stringsAsFactors=FALSE)
```
...
Every row in this table represent a coral genus, which is designated with in the column ``genus.proper``.
...
dats <- merge(dats, sym, by.x="genus", by.y="genus.proper", all.x=TRUE)
...
# legend function
legZAZ <- function(...) 
  legend(legend=c("z", "az"), col=c("blue", "orange"), lty=c(1,1), bg="white", cex=1.3, ...)
legZAZ("topleft")
```
