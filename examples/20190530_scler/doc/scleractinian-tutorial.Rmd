...
# Data download
To run analyses, first the occurrence data have to be downloaded and processed. You can download all occurrences of Scleractininan corals by pasting the following lines of code to your browser.
...
As this format is relatively inefficient, it is better practice to store the data in a binary format. You can read this file with the following line of code:

```
dat <- read.csv("D:/pbdb_data.csv", sep=",", header=TRUE, stringsAsFactors=FALSE)
```
save it exactly the way it was imported with a  ``save()`` function:
```
...
After you have set up your working directory, you can read the data with the ``load()`` function, or you can get my copy from GitHub.


```{r load, echo= TRUE, eval=TRUE}
load(url("https://github.com/divDyn/assets/raw/master/examples/20190530_scler/data/20190530_scler.RData"))
```
...
The analyses will be conducted at the level of genera. The entries that have no valid genus names in the PaleoDB can be omitted at this stage. They will not provide us any information about the evolution of the group.
...
legend("topright", legend=c("extinction", "origination"), col=c("red", "green"), lwd=2)
```
...
sym <- read.csv(
"https://github.com/divDyn/assets/raw/master/examples/20190530_scler/data/coral_genus_traits.csv",
...
dats <- merge(dats, sym, by="genus", all.x=TRUE)
